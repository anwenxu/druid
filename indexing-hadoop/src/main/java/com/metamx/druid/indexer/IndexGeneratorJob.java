...
public class IndexGeneratorJob implements Jobby {
	...
	public boolean run() {
		try {
			Job job = new Job(new Configuration(), String.format(
	...
	public static List<DataSegment> getPublishedSegments(
		...
		try {
			...
			for (FileStatus status : fs.listStatus(descriptorInfoDir)) {
				...
				log.info("Adding segment %s to the list of published segments",
...

	{
		...
		protected void innerMap(InputRow inputRow, Text text, Context context)
			...
			context.write(
					new SortableBytes(bucket.get().toGroupKey(), Longs
	...
	public static class IndexGeneratorPartitioner extends
		...
		@Override
		public int getPartition(BytesWritable bytesWritable, Text text,
				int numPartitions) {
	...
	public static class IndexGeneratorReducer extends
		...
		protected void setup(Context context) throws IOException,
			...
			config = HadoopDruidIndexerConfig.fromConfiguration(context
					.getConfiguration());
		...
		protected void reduce(BytesWritable key, Iterable<Text> values,
			...
			for (final Text value : values) {
				...
				if (numRows >= rollupSpec.rowFlushBoundary) {
					log.info("%,d lines to %,d rows in %,d millis", lineCount
							- runningTotalLineCount, numRows,
							System.currentTimeMillis() - startTime);
					runningTotalLineCount = lineCount;

					final File file = new File(baseFlushFile, String.format(
			...
			if (toMerge.size() == 0) {
				...
				IndexMerger.persist(index, interval, mergedBase,
						new IndexMerger.ProgressIndicator() {
							@Override
							public void progress() {
								context.progress();
				...
				if (!index.isEmpty()) {
					...
					IndexMerger.persist(index, interval, finalFile,
							new IndexMerger.ProgressIndicator() {
								@Override
								public void progress() {
									context.progress();
				...
				mergedBase = IndexMerger.mergeQueryableIndex(indexes, aggs,
		...
		private void serializeOutIndex(Context context, Bucket bucket,
			...
			Path indexBasePath = config.makeSegmentOutputPath(fileSystem,
					bucket);
			Path indexZipFilePath = new Path(indexBasePath, String.format(
					"index.zip.%s", attemptNumber));
			final FileSystem infoFS = config.makeDescriptorInfoDir()
					.getFileSystem(context.getConfiguration());
			final FileSystem outputFS = indexBasePath.getFileSystem(context
					.getConfiguration());
			...
			if (outputFS instanceof NativeS3FileSystem) {
				loadSpec = ImmutableMap.<String, Object> of("type", "s3_zip",
				...
				loadSpec = ImmutableMap.<String, Object> of("type", "hdfs",
						"path", indexOutURI.getPath());
			} else {
				throw new ISE("Unknown file system[%s]", outputFS.getClass());
			}

			DataSegment segment = new DataSegment(config.getDataSource(),
			...
			for (int i = 0; i < 6; i++) {
				if (renameIndexFiles(infoFS, outputFS, indexBasePath,
						indexZipFilePath, finalIndexZipFilePath, segment)) {
					log.info("Successfully renamed [%s] to [%s]",
							indexZipFilePath, finalIndexZipFilePath);
					success = true;
					break;
				} else {
					log.info("Failed to rename [%s] to [%s]", indexZipFilePath,
			...
			if (!success) {
				if (!outputFS.exists(indexZipFilePath)) {
					throw new ISE("File [%s] does not exist after retry loop.",
							indexZipFilePath.toUri().getPath());
				}
		...
		private boolean renameIndexFiles(FileSystem intermediateFS,
			...
			writeSegmentDescriptor(outputFS, segment, new Path(indexBasePath,
					"descriptor.json"));
		...
		private void writeSegmentDescriptor(FileSystem outputFS,
				DataSegment segment, Path descriptorPath) throws IOException {
			...
			try {
				HadoopDruidIndexerConfig.jsonMapper.writeValue(descriptorOut,
						segment);
			} finally {
				descriptorOut.close();
			}
		...
		private long copyFile(Context context, ZipOutputStream out,
				File mergedBase, final String filename) throws IOException {
		...
		private IncrementalIndex makeIncrementalIndex(Bucket theBucket,
				AggregatorFactory[] aggs) {
			return new IncrementalIndex(new IncrementalIndexSchema.Builder()
